...
1. Создать в корне проекта файл .env
   ...
   Для каждого параметра можно указать собственное значение либо скопировать готовые значения 
   из .env.example без изменений:
   ...
   Если изменяется DB_PORT, необходимо также указать тот же порт в DB_URL.
